...
export function classifyLandmarksRuleBased(landmarks) {
  if (!isValidLandmarks(landmarks)) return { sign: null, confidence: 0 }

  const extended = TIP_IDS.map((tipId, idx) => isFingerExtended(landmarks, tipId, PIP_IDS[idx]))
